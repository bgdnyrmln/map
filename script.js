...
// Initialize the map
var map = L.map('map').setView([56.95, 24.11], 10);
...
fetch('data.json')
    ...
    .then(data => {
        data.features.forEach(feature => {
            // Get LKS-92 coordinates from GeoJSON feature
            const lks92Coordinates = feature.geometry.coordinates;

            // Convert the LKS-92 coordinates (x, y) to WGS84 (lat, lng)
            const latLng = LKS92WGS84.convertXYToLatLon(lks92Coordinates);

            // Add a marker for each place to the map
            L.marker([latLng[0], latLng[1]])
                .addTo(map)
                .bindPopup(`<strong>${feature.properties.PLACENAME}</strong><br>Latitude: ${latLng[0].toFixed(6)}<br>Longitude: ${latLng[1].toFixed(6)}`);
        });
    })
